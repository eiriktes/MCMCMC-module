...
function load_mc_model(filename::String)
    ...
    etat = eval(Meta.parse(split(model[4],"::")[2]))
    modeles = eval(Meta.parse(split(model[5],"::")[2]))
    valide = eval(Meta.parse(split(model[6],"::")[2]))
    data = eval(Meta.parse(split(model[7],"::")[2]))

    mcmcmc_result(data, succes, chain_nb, etat, modeles, valide, nb_chaud, fixed)
end
